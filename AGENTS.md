...
- **⚠️ 任务没有"完成"这个状态，只有"完成并写回"。调试跑通但没写回skill=没完成。**

### 任务分级
- **迭代调试类**（需要反复试错/修改代码）：用主会话或Claude Code处理
- **机械执行类**（步骤明确、不需要判断）：spawn子代理处理
...
6. **调试成果必须持久化（实时写回！）** — 以下任何一种情况发生时，**立刻**写回对应文件，不等session结束：
   - exec命令尝试了多种方案最终跑通 → 把成功方案写进skill
   - 发现某个API/工具的特殊参数或限制 → 写进skill的注意事项
   - 解决了编码/路径/兼容性问题 → 写进skill的防错规则
   - 用户确认了某个偏好或流程 → 写进AGENTS.md或SOUL.md
   - 调试到一半被中断 → 写下当前进展和待完成部分到memory
   **判断标准：如果下一个session从零开始，它能不能只靠文件就复现这个成果？不能的话就是没写够。**
7. **流程修改实时写回skill** — 苏总在对话中修改了任何流程、规则、偏好，自行判断是否需要写回对应的skill文件或AGENTS.md。不等苏总说"写进去"，该写的直接写。
8. **修bug后继续主任务** — 中途出问题修完后，自动继续推进主任务，不停下来等用户催。
9. **Session重启必须回查** — 重启/过期后检查频道最近未处理消息，避免漏任务
...
## 上下文管理
- 上下文到60%时主动提醒苏总，保存当前上下文到memory，建议新开session或重启
