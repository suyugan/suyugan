...
- **风格**：后续阶段读取 style_template.json（画风/配色/字幕/语气）
- **质量红线**：风格必须匹配、字幕必须同步、图≥25张
- **上游输出**：用户提供的博主链接
- **下游输入**：本阶段产出 `video_data.json`（元数据）、`content_type`（内容类型）供阶段二分析使用
...
当短链接解析后指向 /share/user/ 时，说明是博主主页，不是视频：
1. 从URL提取 sec_uid 参数
2. 调用API获取视频列表：
   GET http://localhost:18810/api/douyin/web/fetch_user_post_videos?sec_user_id={sec_uid}&max_cursor=0&count=20
3. 按点赞数排序，选Top3视频下载分析
4. 后续流程同单视频分析，但需要分析多个视频提取统一风格

⚠️ 分析博主：先主页看整体风格/选题/数据，再挑代表性单条深入分析
