...
When `--extract` is used, skill files are parsed from the concatenated content response and written to individual files under `/tmp/openclaw-skills/{skill-name}/`.

The agent can then read and execute these files directly.
