#!/usr/bin/env bun
/**
 * XHS (Xiaohongshu/RedNote) image generator using Google Gemini/Imagen APIs
 * Optimized for social media infographics with 3:4 or 1:1 aspect ratios
 *
 * Usage:
 *   npx -y bun generate-image.ts --prompt "prompt text" --image output.png
 *   npx -y bun generate-image.ts --promptfile prompt.md --image output.png
 *   npx -y bun generate-image.ts --promptfile prompt.md --image output.png --ar 1:1
 */
...
function printUsage(): void {
  console.log(`XHS Image Generator - Optimized for Xiaohongshu/RedNote infographics

Usage:
  npx -y bun generate-image.ts --prompt "A cute infographic..." --image slide.png
  npx -y bun generate-image.ts --promptfile prompt.md --image slide.png
  npx -y bun generate-image.ts --promptfile prompt.md --image slide.png --ar 1:1
...
Options:
  ...
  --ar <ratio>              Aspect ratio (default: 3:4, supports 1:1, 4:3, 16:9)
...
XHS Optimized Aspect Ratios:
  3:4   - Standard XHS post (default, recommended)
  1:1   - Square format
  4:3   - Landscape alternative
  9:16  - Full screen story`);
}
