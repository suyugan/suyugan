...
COVER_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cover_dashan.png")
...
CONTENT = """
<section style="max-width:680px;margin:0 auto;padding:20px;font-size:16px;line-height:2;color:#333;">
...
<p><em>"你好，我觉得你很有气质……"</em><br/>
<em>"请问这个座位有人吗？"（明明整排都空着）</em><br/>
<em>"你也在做设计吗？我也是……"（我明明是写代码的）</em></p>
...
<p><strong>场景一：咖啡店</strong><br/>
看到对方在看一本书→"这本书我一直想看，值得买吗？"<br/>
看到对方点了一杯很特别的饮品→"你这杯是什么？看起来不错。"</p>

<p><strong>场景二：书店/展览</strong><br/>
"你也喜欢XXX？我觉得他的XX作品最好。"<br/>
"这个展你看了多久了？有没有特别推荐的？"</p>

<p><strong>场景三：健身房/运动场</strong><br/>
"你这个动作做得很标准，练了多久了？"<br/>
"你这双鞋不错，跑步穿舒服吗？"</p>
...
<p>❌ <strong>堵路式搭讪</strong>：拦住人家的去路，逼人跟你对话。这不是搭讪，这是拦截。<br/>
❌ <strong>评价身材/外貌</strong>："你身材真好""你腿好长"——你觉得是赞美，人家觉得是骚扰。<br/>
❌ <strong>纠缠不放</strong>：对方明显不想聊了，你还在那儿找话题。察言观色，是成年人的基本功。<br/>
❌ <strong>立刻要微信</strong>：聊了不到一分钟就"加个微信呗"，目的性太强了。自然地聊，聊开心了再顺势交换联系方式。<br/>
❌ <strong>天黑了还搭讪</strong>：晚上在路上被陌生人叫住，换谁都会有警惕心。时间地点要选对。</p>
...
<p style="text-align:center;color:#888;font-size:14px;">写这篇的时候，我又想起三年前星巴克那个下午。<br/>
如果时间能倒流，我大概会走过去说：<br/>
<em>"你Figma用的什么字体？挺好看的。"</em><br/><br/>
就这么简单。</p>
...
# === Step 2: 生成封面图HTML并截图 ===
print("2. 生成封面图...")
cover_html = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cover_dashan.html")
with open(cover_html, "w", encoding="utf-8") as f:
    f.write("""<!DOCTYPE html>
<html><head><meta charset="utf-8">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:900px;height:383px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);
font-family:'Microsoft YaHei','PingFang SC',sans-serif;display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative}
.bg-circle{position:absolute;border-radius:50%;opacity:0.1;background:white}
.c1{width:300px;height:300px;top:-80px;right:-60px}
.c2{width:200px;height:200px;bottom:-60px;left:-40px}
.c3{width:150px;height:150px;top:50%;left:60%;transform:translate(-50%,-50%)}
.content{position:relative;z-index:1;text-align:center;color:white;padding:40px}
.title{font-size:52px;font-weight:900;letter-spacing:4px;text-shadow:2px 2px 20px rgba(0,0,0,0.3);margin-bottom:16px}
.sub{font-size:22px;color:rgba(255,255,255,0.85);font-weight:400;letter-spacing:2px}
.tag{margin-top:20px;display:inline-block;font-size:14px;background:rgba(255,255,255,0.2);
padding:6px 20px;border-radius:20px;color:rgba(255,255,255,0.9);letter-spacing:1px}
</style></head>
<body>
...
<div class="content">
<div class="title">搭讪这件小事</div>
<div class="sub">三年踩坑经验，不油腻社交指南</div>
<div class="tag">🤝 社交破冰 · 实用干货</div>
</div>
</body></html>""")
print(f"   HTML已生成: {cover_html}")

# 输出路径供后续使用
print(f"COVER_HTML={cover_html}")
